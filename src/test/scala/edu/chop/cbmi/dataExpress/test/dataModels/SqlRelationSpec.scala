...
class SqlRelationSpec extends PresidentsSpecWithSourceTarget{
  ...
  describe("A SqlRelation") {
    it("should result from a call to DataTable") {
      ...
      when("accessed via head and tail it should give a DataRow and a new Iterable[DataRow[Option[Any]]] respectively")
      val row1 = sr.head
      ...
      val other_rows = sr.tail
      val row2 = other_rows.head
      ...
      val sum_terms = (sr.head.num_terms.asu[Int] /: sr.tail)(_ + _.num_terms.asu[Int])
